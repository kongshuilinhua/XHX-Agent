...
- 如果已有 pending confirm，`/allow` 允许当前等待的命令一次。
- 如果已有 pending confirm，`/deny` 拒绝当前等待的命令一次。
- 如果没有 pending confirm，`/allow` 设置下一次 confirm 级命令允许一次。
- 如果没有 pending confirm，`/deny` 设置下一次 confirm 级命令拒绝一次。
- 无全屏 UI 可交互等待时默认拒绝。
...
- `xhx tui --fullscreen` 当前 `/verify` 和 `/repair` 也会启动后台 Runtime worker，因此 confirm 级验证命令可以在窗口内等待 `/allow` 或 `/deny`。
...
- `xhx tui --fullscreen` 遇到 confirm 级权限时会显示 pending confirm；可用 `/allow` 或 `/deny` 响应当前等待的命令。没有 pending confirm 时，`/allow` 或 `/deny` 会设置下一次 confirm 决策；非全屏路径仍默认拒绝。
