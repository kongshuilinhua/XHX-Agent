...
- 新增 `xhx tui --fullscreen` 实验性 Textual 窗口，可以渲染 `ConsoleState` 的 header、conversation、runtime、changed files 和 command hints，支持普通任务执行事件刷新、`/model`、`/plan`、`/context`、`/evidence`、`/diff`、`/verify`、`/repair`、`/skills`、`/mode`、`/dashboard`、`/cancel`、一次性 `/allow` / `/deny` 权限确认，并支持 `/help`、`/status`、`/clear`、`/exit` 本地命令。
...
- Textual 全屏路径目前仍是实验性 shell，已支持普通任务执行、`/model`、`/plan`、`/context`、`/evidence`、`/diff`、`/verify`、`/repair`、`/skills`、`/mode`、`/dashboard`、`/cancel` 和一次性权限确认，但还没有接入 `/live` 和运行中 steer。
