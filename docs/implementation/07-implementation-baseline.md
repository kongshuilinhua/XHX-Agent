...
- 新增 `xhx tui --fullscreen` 实验性 Textual 窗口骨架，可以渲染 `ConsoleState` 的 header、conversation、runtime、changed files 和 command hints，并支持 `/help`、`/status`、`/clear`、`/exit` 本地命令。
...
- Textual 全屏路径目前只是实验性 shell，只支持少量本地命令，还没有接入完整任务执行、全部 slash 命令路由和权限确认。
