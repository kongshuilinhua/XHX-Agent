...
1. 用户指定命令。
2. 如果 Repo Intelligence 能映射到 JS/TS 测试文件，并且 `test` 脚本是 `vitest`、`jest` 或 `node --test`，使用 `npm test -- <test-file>`。
3. 未识别 test runner 或没有 impacted test 时，使用 `npm test`。
4. `npm run typecheck`。
5. `npm run build`。
...
- 不猜测未知 test runner 的参数协议。
- package.json 必须能被解析为 JSON；解析失败时不从该文件推断命令。
- JS/TS targeted test 仍来自 direct test 命名和有限 import graph，不代表完整调用图。
