...
1. **Context Pack Compiler (上下文编译内核)**：
   ...
   - 📖 详细设计请参阅私密导读：[01-Context Pack Compiler 源码深潜](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/01-context-pack-compiler.md)
...
2. **Evidence Runtime (证据追踪与审计内核)**：
   ...
   - 📖 详细设计请参阅私密导读：[02-Evidence Runtime 源码深潜](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/02-evidence-runtime.md)

3. **Safe Execution Kernel (安全执行内核)**：
   统一拦截与管理所有的工具调用，进行策略检查（Policy Check）与命令风险评级。在修改前自动创建轻量级 Git Checkpoint 备份，并提供安全的失败回滚复原方案。
   - 📖 详细设计请参阅私密导读：[03-Safe Execution Kernel 源码深潜](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/03-safe-execution-kernel.md)
...
4. **Adaptive Planner & Parallel DAG (自适应规划与并行拓扑调度内核)**：
   ...
   - 📖 详细设计请参阅私密导读：[04-DAG Planner 源码深潜](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/04-dag-planner.md)
...
- 📂 [Context Pack Compiler (上下文编译内核)](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/01-context-pack-compiler.md)：探索如何在有限 Token 预算下极致裁剪上下文，防止大模型幻觉与信息溢出。
- 📂 [Evidence Runtime (证据追踪与审计内核)](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/02-evidence-runtime.md)：剖析 Raw Trace 记录落盘、TrailGraph 拓扑推理与结构化审计报告渲染。
- 📂 [Safe Execution Kernel (安全执行内核)](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/03-safe-execution-kernel.md)：深入子进程沙箱隔离、命令风险拦截、Git 轻量备份与自愈回滚物理机制。
- 📂 [Adaptive Planner & Parallel DAG (自适应规划与并行拓扑调度内核)](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/04-dag-planner.md)：剖析基于 Kahn 算法的拓扑排序、读写分离多线程并发调度及 2-Turn 自愈修复环路。
- 📂 [Repo Intelligence Graph (代码智能与 SQLite 符号索引内核)](file:///d:/pycharmprojects/XHX-Agent/docs/deep_dive/05-repo-intelligence.md)：解构抽象语法树（AST）提取、TS/JS 路径别名猜测解析、SQLite 索引 B-Tree 加速及测试影响分析。
