...
- **历史压缩 (compaction)**：自主 `loop` 跑多轮时，溢出最近窗口的旧工具摘要会被压缩为一条保留（默认启发式统计：工具调用次数 + 失败计数；autonomous 模式下可用当前 profile 的模型生成语义摘要，失败自动回退启发式），帮助长 loop 维持长程一致性（`_compact_tool_summaries`）。
...
> **实现现状（M2，2026-06-08）**：抽象层、registry 选择、`run_task(mode=...)` API 已就位。`loop` 已实现为**自主统一循环**——模型持续多轮 read→edit→verify，直到自报完成或达 `max_loop_turns`（默认 20），而非改一个文件就停；自动分类兜底仍走 `linear`（改动后即停，向后兼容）。`graph` 已实现为 LangGraph 多 agent 工作流（M3）；`--mode` 已接通三个入口——CLI `xhx run`、console 与 textual 全屏的 `/mode`（M4，全部完成）。见 `docs/implementation/20-implementation-baseline.md`。
