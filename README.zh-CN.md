...
- **如实交代边界。** [实现状态](#实现状态)如实区分"完整"与"简化"，[工程笔记](#工程笔记)记录了真实模型下哪些东西崩了、以及哪些**光靠 prompt 修不了**。
...
三条比"测试全绿"更值钱的发现——每条都是**真实模型**偏离舒适的离线 mock 的地方。

**1 · `apply_patch` 撞上真实模型。** patch 工具最初围绕自定义 `*** Begin Patch … *** End Patch` 信封构建，mock 也老实产出它。换成真 DeepSeek，*每次*编辑都失败：`Patch must start with *** Begin Patch`。真实模型产出的是 **unified diff**——常包在 ```` ```diff ```` 围栏里。修法是按*格式*分派：信封、unified diff（`---`/`+++`/`@@`，`/dev/null` = 新文件）、外加一个剥围栏的预处理。**教训：** mock 一致不等于真实一致；模型的输出分布*就是*你要解析的规范。

**2 · prompt 不是银弹。** `Agent` 工具让模型把聚焦调研交给隔离的只读子 agent、保持父上下文干净。该能力已接好、门控、正确——但即便 prompt 明确引导，真实模型大多还是自己读文件、很少去用它。如实记下：**改变模型行为常常需要比"system prompt 里一段话"更强的机制。**

**3 · 注入类特性不到"只存于记忆的事实真的改变了输出"那一刻就不算真。** 跨会话召回易于*接通*、也易于自欺。所以测试不是"召回函数返不返回行"——而是：写一条**只**存在于 `.xhx/memory/` 的事实（项目吉祥物是一只叫 Pacha 的蓝色蝾螈），问真实模型一个本来答不出的问题，确认召回的事实既到了 prompt、又塑形了答案。**教训：** 凡是会静默注入上下文的东西，都要用一条模型本来不可能知道的事实做端到端验证。
...
- TUI 渲染内部有意少做单测（由 pilot 冒烟测试覆盖），这也是覆盖率下限定在 75% 而非更高的原因。
