...
- **多模型路由 + 优雅 fallback。** 按角色路由到不同 profile，主模型出错/限流时沿 profile 链降级；与流式正交。`xhx benchmark --profiles default,cheap` 在同一任务集上量化对比 profile 的成功率 / 轮数 / token / 耗时。
- **持久化证据链 + 确定性回放。** 每轮模型调用与每次工具执行都落 `.xhx/traces/<run_id>.jsonl`（真实 token 用量、参数、结果摘要）；`xhx replay <run-id>` 离线重建一次运行的轮数、命令与最终回答——长任务"疑似卡死"时看 trace 就知道它在干什么。
...
| **证据链 / 回放** | 逐轮 `model_turn` + 逐工具 `tool_call`/`tool_result` 写 `.xhx/traces/`（与会话索引共用 run_id）；`xhx replay` 离线重建运行全貌。 |
...
| `--json` | 以结构化 JSON 输出结果。 |
| `--continue` | 从最近一次会话恢复：优先全量还原完整对话历史，缺 transcript 的老会话回退注入摘要。 |
| `--resume <run-id>` | 从指定历史会话恢复（`xhx sessions` 可列出），同样优先全量还原。 |

其他命令：`init`（`--global` 写用户级 `~/.xhx/`）、`repo-index`、`sessions`、`tui` / `chat`、`rpc`（stdio 上的 JSON-RPC 2.0）、`replay <run-id>`（从 trace 离线重建一次运行）、`benchmark`（`--profiles default,cheap` 同任务集对比多个模型 profile）、`memory`、`compact`、`config list` / `config set-profile`。
...
- 证据链与回放：统一主循环逐轮/逐工具落 `.xhx/traces/<run_id>.jsonl`（trace、会话索引、`xhx sessions` 共用 run_id），`xhx replay` 离线重建轮数/token/命令/最终回答；`--continue`/`--resume` 全量还原完整对话，缺 transcript 的老会话回退摘要续接。
- Benchmark：`--profiles` 在同一任务集上对比多个模型 profile（成功率/轮数/token/耗时，全部真实运行数据）。
...
```bash
uv run pytest          # 测试套件（覆盖率 ~75%，CI 门槛 70%）
