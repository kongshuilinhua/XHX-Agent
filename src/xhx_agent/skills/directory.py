...
def parse_tool_json(path: Path) -> list[dict[str, Any]]:
    try:
        ...
        logger.warning("Failed to parse tool.json at %s: %s", path, e)
        return []

    if isinstance(raw, dict):
        raw = [raw]
    if not isinstance(raw, list):
        logger.warning("tool.json at %s must be a JSON array or object", path)
        return []
...
def load_tool_implementation(references_dir: Path, tool_name: str) -> Callable[..., Any] | None:
    ...
    if spec is None or spec.loader is None:
        logger.warning("Cannot create module spec for %s", script)
        return None
    ...
    try:
        ...
        logger.warning("Failed to load tool implementation %s: %s", script, e)
        return None

    execute_fn = getattr(module, "execute", None)
    if execute_fn is None:
        logger.warning("Tool implementation %s has no 'execute' function", script)
        return None
...
def register_skill_tools(skill_dir: Path, registry: ToolRegistry) -> int:
    ...
    for schema in schemas:
        ...
        if not tool_name:
            logger.warning("Skipping tool with no name in %s", tool_json_path)
            continue

        if registry.get(tool_name) is not None:
            logger.debug("Tool '%s' already registered, skipping", tool_name)
            continue
        ...
        if impl is None:
            logger.warning("No implementation for tool '%s' in %s", tool_name, references_dir)
