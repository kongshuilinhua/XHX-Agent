...
| v0.6 Repo Intelligence Graph | 部分实现 | 已有 repo map、基础符号提取、symbol search、context builder、轻量 import graph、递归依赖测试影响面分析和 `.xhx/repo/index.json`；Context Pack / Verification Router 优先复用落盘索引，索引过期时重建，并能选择 import graph 邻接符号上下文；完整 Tree-sitter、SQLite 索引、调用图和引用图仍未完成。 |
