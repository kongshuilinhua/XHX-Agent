...
| v0.5 TUI / Command Console | 部分实现 | 已有 `xhx tui` / `xhx chat` Rich 命令控制台、`xhx tui --fullscreen` 实验性 Textual 窗口任务执行、全屏 `/help` / `/status` / `/verify` / `/allow` / `/deny` / `/clear` / `/exit`、`/dashboard` 页面、`/live` Rich Live 动态仪表盘、`/verify` 手动验证、`/repair` 默认一轮修复、`/repair loop` 最多两轮手动修复、`/diff` 只读 git diff 摘要、`/cancel` 阶段边界取消、follow-up 上下文包装、权限确认、Runtime 事件流、模型增量输出、ConsoleState 归约和状态摘要视图；全屏 Textual 路径仍未接入全部 slash 命令和运行中实时 steer。 |
