...
当前代码按 [版本实施基线](docs/implementation/07-implementation-baseline.md) 推进。当前重点是收尾 **v0.1-C 验证闭环**：默认 mock profile 可以离线跑通 read/search、apply_patch、验证推断、交互确认 / `--yes` 执行、Raw Trace、Evidence Index 和 Markdown 报告；同时已有真实模型计划解析、工具结果反馈循环、dry-run 入口和原子化 `apply_patch` 写入内核。这些能力仍不等于完整自动编码 Agent，不要把当前版本理解为已经能在任意真实仓库中自动修代码。
...
| v0.1-A 真实模型接入 | 部分实现 | 已有 OpenAI-compatible 非流式请求、API key 环境变量读取、mock model、JSON plan 解析和结构化错误诊断；流式输出和 provider response format 仍未完成。 |
| v0.1-B Tool-call loop | 部分实现 | 已有 Tool Registry、read/search/apply_patch、工具结果反馈循环、patch evidence 和原子化 patch；工具层尚未完全统一进入 Safe Execution Kernel。 |
| v0.1-C 验证闭环 | 部分实现 | 已有 Python/Node 验证推断、交互确认 / `--yes` 执行、验证 trace/evidence、只读任务跳过验证、失败停止和命令输出摘要；更细的 changed-file 到测试映射仍需继续增强。 |
| Context Pack Compiler 最小版 | 已实现 | 每轮模型调用前生成预算化上下文包，包含项目摘要、`XHX.md`、变更文件、工具结果摘要和证据摘要。 |
| 工具结果反馈循环 | 部分实现 | Runtime 可把上一轮工具结果摘要反馈给下一轮模型；目前仍是最小循环，不包含 repair loop 和 DAG。 |
...
| OpenAI-compatible 真实模型调用 | 部分实现 | 已有非流式 Chat Completions 计划请求、Context Pack 输入、API key 环境变量读取、结构化错误处理、`--dry-run` 计划预览和最多 4 轮工具反馈；仍要求模型按 JSON 计划协议返回工具步骤，不等于通用自动修代码。 |
