...
- **A real permission system.** Tools are classified `read` / `write` / `command` and gated by mode (`default` / `acceptEdits` / `plan` / `bypassPermissions`) plus a three-tier `allow` / `ask` / `deny` rule engine (user → project → local, `deny > ask > allow`) with Bash command-prefix / subcommand matching, dangerous-command detection, sensitive-path guarding (`.env` / `.git` / shell configs stay bypass-immune), and a path sandbox. **Plan mode is two-stage**: the agent researches read-only, presents a plan, and only executes after you approve.
...
- Permission system: `default` / `acceptEdits` / `plan` / `bypassPermissions` modes, three-tier `allow` / `ask` / `deny` rule engine (user → project → local, `deny > ask > allow`, Bash prefix/subcommand matching), path sandbox, dangerous-command detection, sensitive-path guarding, two-stage plan mode, inline TUI approval dialogs.
