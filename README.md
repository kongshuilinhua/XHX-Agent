...
* **Pluggable Orchestrator**: A unified `Orchestrator` abstraction drives a task with either a single autonomous **`loop`** (Claude-Code-style) or a multi-agent **`graph`**, selectable via the `run_task(mode=...)` API over one shared tool/safety/context base. The **`loop`** mode is **implemented and autonomous** — the model keeps iterating (read → edit → verify) across up to `max_loop_turns` (default 20) until it reports done, instead of stopping after the first change; the auto-classification fallback still uses `linear` (stop-on-first-change) for backward compatibility. *Roadmap:* a LangGraph-backed `graph` (M3) and `--mode` CLI/TUI wiring (M4). See `docs/implementation/20-implementation-baseline.md`.
