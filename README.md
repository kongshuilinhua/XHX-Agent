...
| v0.5 TUI / Command Console | 部分实现 | 已有 `xhx tui` / `xhx chat` Rich 命令控制台、`/` 命令、`/dashboard` 页面、`/verify` 手动验证、权限确认、Runtime 事件流、ConsoleState 归约和状态摘要视图；仍不是全屏 Textual TUI，也没有真正 token 级模型流式输出。 |
