...
| `xhx repo-index` | 已实现 | 只读输出 repo index 的 missing / invalid / stale / current 状态、大小、条目数、指纹和 reference 截断情况；不会自动重建索引。 |
