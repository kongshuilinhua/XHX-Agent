...
| v0.5 TUI / Command Console | 部分实现 | 已有 `xhx tui` / `xhx chat` Rich 命令控制台、`/` 命令、权限确认和最近运行状态；仍不是全屏 Textual TUI，也没有真正流式模型输出。 |
