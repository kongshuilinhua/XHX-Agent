...
| v0.5 TUI / Command Console | 部分实现 | 已有 `xhx tui` / `xhx chat` Rich 命令控制台、`/` 命令、`/dashboard` 页面、`/verify` 手动验证、`/repair` 一轮手动修复、`/cancel` 阶段边界取消、follow-up 上下文包装、权限确认、Runtime 事件流、模型增量输出、ConsoleState 归约和状态摘要视图；仍不是全屏 Textual TUI，也没有固定区域动态刷新或运行中实时 steer。 |
