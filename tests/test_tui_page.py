...
def test_render_console_page_shows_runtime_sections() -> None:
    ...
    state.reduce(
        RuntimeEvent(
            ...
            payload={"turn": 1, "selected": 2, "omitted": 1, "used_tokens_estimate": 100, "budget_tokens": 6000},
        )
    )
    state.reduce(
        RuntimeEvent(type="model_plan", message="Patch calculation", payload={"status": "continue", "step_count": 1})
    )
    state.reduce(RuntimeEvent(type="model_delta", message='{"summary":"Patch calculation"}', payload={"turn": 1}))
