...
def test_repo_intel_diagnostics_reports_current_index(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "calc.py").write_text("def add(a, b):\n    return a + b\n", encoding="utf-8")
...
def test_repo_intel_diagnostics_reports_stale_index(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir()
    ...
    assert "fingerprint" in diagnostics.reason
...
def test_search_symbols_via_sqlite(tmp_path: Path) -> None:
    ...
    from xhx_agent.repo_intel.repo_map import build_repo_map
    from xhx_agent.repo_intel.symbols import Symbol, SymbolIndex, search_symbols
    from xhx_agent.repo_intel.types import CallGraph, ImportGraph, ReferenceIndex, RepoIntelIndex

    mock_symbols = [
        Symbol(
            name="my_test_func", kind="function", path="demo.py", line=10, end_line=15, language="python", parent=None
        ),
        Symbol(
            name="another_func", kind="function", path="demo.py", line=20, end_line=25, language="python", parent=None
        ),
    ]
